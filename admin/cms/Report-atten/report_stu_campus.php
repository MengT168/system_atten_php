...
<body>
    <main id="main" class="main">
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Absent Report By Campus</h5>
                            <div class="row g-2">
                                <div class="col-3">
                                    <label class="form-label">Campus</label>
                                    <select id="campus" name="academicpro" class="form-select">
                                ...
                                <div class="col-2 hidden" id="refreshBtnContainer">
                                    <a id="searchNameBtn" style="margin-top: 30px;" href="?tag=campus_stu_report_atten&op=search&campus=<?php echo $campus = isset($_GET['campus']) ? $_GET['campus'] : null; ?> "class="btn btn-primary">Refresh</a>
                                </div>
        ...
        <?php
        if (isset($_GET['op']) && $_GET['op'] == 'search' && isset($_GET['tag']) && $_GET['tag'] == 'campus_stu_report_atten') {
            ...
            if ($campus !== null) {
                $query = "SELECT attendance_tbl.Attended, studentinfo_tbl.NameInLatin, faculty_tbl.FacultyEN, major_tbl.MajorEN, semester_tbl.SemesterEN, 
                          ...
                          WHERE attendance_tbl.Attended = 1 AND campus_tbl.CampusID=$campus";
                ...
                if ($result) {
                    ...
                    echo '<div class="text-center"><h1>STUDENT ABSENT BY CAMPUS</h1></div>';
    ...
    <script>
        document.getElementById('searchMajorBtn').addEventListener('click', function(event) {
            ...
            var url = '?tag=campus_stu_report_atten&op=search';
        ...
        document.getElementById('searchNameBtn').addEventListener('click', function(event) {
            ...
            var campus = document.getElementById('campus').value;
            if (!campus) {
                swal("Error", "Please select a campus before searching by name.", "error");
                return;
            }
        ...
        var currentBatch = new URLSearchParams(window.location.search).get('campus');
        if (currentBatch) {
            document.getElementById('campus').value = currentBatch;
        }
    </script>
</body>
</html>