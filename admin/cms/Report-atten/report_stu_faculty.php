...
<body>
    <main id="main" class="main">
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Absent Report By Faculty</h5>
                            <div class="row g-2">
                                <div class="col-3">
                                    <label class="form-label">Faculty</label>
                                    <select id="faculty" name="academicpro" class="form-select">
                                ...
                                <div class="col-2 hidden" id="refreshBtnContainer">
                                    <a id="searchNameBtn" style="margin-top: 30px;" href="?tag=faculty_stu_report_atten&op=search&faculty=<?php echo $faculty = isset($_GET['faculty']) ? $_GET['faculty'] : null; ?> "class="btn btn-primary">Refresh</a>
                                </div>
        ...
        <?php
        if (isset($_GET['op']) && $_GET['op'] == 'search' && isset($_GET['tag']) && $_GET['tag'] == 'faculty_stu_report_atten') {
            ...
            if ($faculty !== null) {
                $query = "SELECT attendance_tbl.Attended, studentinfo_tbl.NameInLatin, faculty_tbl.FacultyEN, major_tbl.MajorEN, semester_tbl.SemesterEN, 
                          ...
                          WHERE attendance_tbl.Attended = 1 AND faculty_tbl.FacultyID=$faculty";
                ...
                if ($result) {
                    ...
                    echo '<div class="text-center"><h1>STUDENT ABSENT BY FACULTY</h1></div>';
    ...
    <script>
        document.getElementById('searchMajorBtn').addEventListener('click', function(event) {
            ...
            var url = '?tag=faculty_stu_report_atten&op=search';
        ...
        document.getElementById('searchNameBtn').addEventListener('click', function(event) {
            ...
            var faculty = document.getElementById('faculty').value;
            if (!faculty) {
                swal("Error", "Please select a faculty before searching by name.", "error");
                return;
            }
        ...
        var currentBatch = new URLSearchParams(window.location.search).get('faculty');
        if (currentBatch) {
            document.getElementById('faculty').value = currentBatch;
        }
    </script>
</body>
</html>