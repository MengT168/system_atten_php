...
<body>
    <main id="main" class="main">
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Absent Report By Major</h5>
                            <div class="row g-2">
                                <div class="col-3">
                                    <label class="form-label">Major</label>
                                    <select id="major" name="academicpro" class="form-select">
                                ...
                                <div class="col-2 hidden" id="refreshBtnContainer">
                                    <a id="searchNameBtn" style="margin-top: 30px;" href="?tag=major_stu_report_atten&op=search&major=<?php echo $major = isset($_GET['major']) ? $_GET['major'] : null; ?> "class="btn btn-primary">Refresh</a>
                                </div>
        ...
        <?php
        if (isset($_GET['op']) && $_GET['op'] == 'search' && isset($_GET['tag']) && $_GET['tag'] == 'major_stu_report_atten') {
            ...
            if ($major !== null) {
                $query = "SELECT attendance_tbl.Attended, studentinfo_tbl.NameInLatin, faculty_tbl.FacultyEN, major_tbl.MajorEN, semester_tbl.SemesterEN, 
                          ...
                          WHERE attendance_tbl.Attended = 1 AND major_tbl.MajorID=$major";
                ...
                if ($result) {
                    ...
                    echo '<div class="text-center"><h1>STUDENT ABSENT BY MAJOR</h1></div>';
    ...
    <script>
        document.getElementById('searchMajorBtn').addEventListener('click', function(event) {
            ...
            var url = '?tag=major_stu_report_atten&op=search';
        ...
        document.getElementById('searchNameBtn').addEventListener('click', function(event) {
            ...
            var major = document.getElementById('major').value;
            if (!major) {
                swal("Error", "Please select a major before searching by name.", "error");
                return;
            }
        ...
        var currentBatch = new URLSearchParams(window.location.search).get('major');
        if (currentBatch) {
            document.getElementById('major').value = currentBatch;
        }
    </script>
</body>
</html>
